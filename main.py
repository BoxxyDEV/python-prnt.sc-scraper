...
while countermax > counter:
    url = "https://prnt.sc/" + abc[random.randint(0, len(abc)-1)] + abc[random.randint(0, len(abc)-1)] + num[random.randint(0, len(num)-1)] + num[random.randint(0, len(num)-1)] + num[random.randint(0, len(num)-1)]
    #generates urls
    try:
        ...
        if req.text.find("i.imgur.com") >= 0:
            print(url)
